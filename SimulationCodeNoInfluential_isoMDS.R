...
for(j in 1:n_sims){
  ...
  tic = Sys.time()
  xTSNE <- MultiPermanova(yes_structure_final[,1:10], maptype = "isoMDS")
  toc = Sys.time() 
  perf_time[[j]] <- toc - tic
...
saveRDS(df2list, paste0(Sys.Date(), "_simulationNoInfluence_isoMDS_list.RDS"))
...
xCMD$plot_map[[1]] %>% plot(pch = 20, col = 1, main = "isoMDS Holdout Plot")
