...
for(j in 1:n_sims){
  ...
  ### Code generates a symmetric matrix based off lower-triangle
  #sig <- matrix(nrow = 60, ncol = 60, sample(c(0,1), 60^2, replace = TRUE))
  # sig <- diag(35, 60)
  # sig[2,1] <- 10
  # Sigma <- makeSymm(sig)
  #diag(Sigma) <- rep(1, nrow(Sigma))
  
  #Matrix::forceSymmetric(sigmat,uplo="L")
  
  # g1 <- rmnorm(n = 10, mean = rep(40, 60), Sigma) %>%
  #   t() %>%
  #   data.frame()
  
  ## Simulate data with correlation between the variables
  # Sigma <- diag(35, 10)
  # Sigma[2,1] <- 20
  # #Sigma[3,1] <- 40
  # 
