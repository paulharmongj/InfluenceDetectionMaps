...
```{r, echo = FALSE}
 g1 <- rmnorm(n = 10, mean = rep(20, 20), diag(35, 20)) %>%
  ...
  datplot <- yes_structure_final %>% mutate(Id = 1:nrow(yes_structure_final), Alpha = ifelse(Group %in% 'Outlier', 1,0.8)) %>%  pivot_longer(1:10) %>% mutate(name = factor(name, levels = c(paste0("X",1:10)))) %>%
    ggplot(aes(name, value, color = Group, group = Id, alpha = Alpha)) + geom_line() + geom_point() + ggtitle("Simulated Data with Outliers Added In") + guides(alpha = FALSE)
datplot
```
...
sammon1 <- readRDS("2022-01-11_simulationNoInfluence_Sammon_list.RDS")
...
#Sammon
ci3 <- sapply(df4list, spuriousInfluence)
cni3 <- 1 - ci3
...
Isomap <- c("","")
Sammon <- c(round(mean(ci3),3),round(mean(cni2),3))
...
mds1 <- readRDS("2022-01-12_simulation1_mds_list.RDS")
tsne1 <- readRDS("2021-10-16_simulation1_tsne_list.RDS")
sammon1 <- readRDS("2022-01-12_simulation1_Sammon_list.RDS")
...
#sammon
ci3 <- sapply(df4list, correctInfluence)
cni3 <- sapply(df4list, correctNonInfluence)
...
MDS <- c(round(mean(ci2),3), round(mean(cni2),3))
Isomap <- c("","")
Sammon <- c(round(mean(ci3),3), round(mean(cni3),3))
...
```{r, echo = FALSE}
 g1 <- rmnorm(n = 10, mean = rep(20, 20), diag(35, 20)) %>%
  ...
  ##### Visualize the data and store in a list
  datplot <- yes_structure_final %>% mutate(Id = 1:nrow(yes_structure_final), Alpha = ifelse(Group %in% 'Outlier', 1,0.8)) %>%  pivot_longer(1:10) %>% mutate(name = factor(name, levels = c(paste0("X",1:10)))) %>% ggplot(aes(name, value, color = Group, group = Id, alpha = Alpha)) + geom_line() + geom_point() + ggtitle("Simulated Data with Outliers Added In") + guides(alpha = FALSE)
datplot
```
...
mds1 <- readRDS("2022-01-12_simulationlarger_mds_list.RDS")
tsne1 <- readRDS("2021-10-17_simulationlarger_tsne_list.RDS")
sammon1 <- readRDS("2022-01-12_simulationlarger_Sammon_list.RDS")
...
g1 <- rmnorm(n = 10, mean = rep(20, 20), diag(35, 20)) %>%
  ...
  add_outliers <- rbind(c(rnorm(k, 1.5, .2), rnorm(10-k, 0, .3))) %>% as_tibble()
  ...
  ##### Visualize the data and store in a list
  datplot1 <- yes_structure_final %>% mutate(Id = 1:nrow(yes_structure_final), Alpha = ifelse(Group %in% 'Outlier', 1,0.8)) %>%  pivot_longer(1:10) %>% mutate(name = factor(name, levels = c(paste0("X",1:10)))) %>% ggplot(aes(name, value, color = Group, group = Id, alpha = Alpha)) + geom_line() + geom_point() + ggtitle("Single Influential Dimension") + guides(alpha = FALSE)
...
k = 4
  ...
  ##### Visualize the data and store in a list
  datplot2 <- yes_structure_final %>% mutate(Id = 1:nrow(yes_structure_final), Alpha = ifelse(Group %in% 'Outlier', 1,0.8)) %>%  pivot_longer(1:10) %>%  mutate(name = factor(name, levels = c(paste0("X",1:10)))) %>% ggplot(aes(name, value, color = Group, group = Id, alpha = Alpha)) + geom_line() + geom_point() + ggtitle("4 Influential Dimensions") + guides(alpha = FALSE)
  
ggarrange(datplot1, datplot2, ncol = 2, nrow = 1)
...
The results are not surprising; for the methods where influential points more heavily impact the shape of the resulting map, the rate of correct detections of influential points increases as the observation differs from the main group on more dimensions. For the MDS maps, detection rates appear to increase up to the point where shifting the individual dimension from one group to another shifts the entire observation's group membership - at that point, at least in a situation where the "influence" from an observation is shifting from one group to a second group, we would expect to see a nearly symmetric decrease in detection rates (since being outlying in 1 dimension and "regular" in 9 dimensions is essentially the opposite scenario of being outlying in 9 dimensions and "regular" in 1).

The PERMANOVA method appears to detect influential points for Sammon maps at a slightly higher rate as the number of features increases, but it does so inconsistently and even decreases from 3 to 4 features.  For t-SNE, there does not appear to be a distinct increase in detection rates based on the sparsity of the dimensionality of the influential point; however, given the performance of the technique previously discussed with t-SNE, this is not unexpected.  
...
df2list <- readRDS("2022-01-16_SparsitySimulation_mds_list.RDS")
df3list <- readRDS("2022-01-16_SparsitySimulation_tsne_list.RDS")
df4list <- readRDS("2022-01-16_SparsitySimulation_sammon_list.RDS")

convertList <- function(listob){
  
  TF <- matrix(TRUE, nrow = length(listob[[1]]), ncol = length(listob))
  templist <- list()
  
  for (i in 1:(length(listob))){
    
    temp <- rep(TRUE, 100)
    ## Identification Rate at given K
    for(j in 1:length(listob[[1]])){
      temp[j] <- tail(listob[[i]][[j]]$PValues,1)<0.05
...
templist <- convertList(df2list)
templist2 <- convertList(df3list)
templist3 <- convertList(df4list)


## create GGplot

temp = tibble(Features = c(1,2,3,4,5), MDS = unlist(templist), TSNE = unlist(templist2), Sammon = unlist(templist3))
temp %>% pivot_longer(cols = c('MDS', 'TSNE', 'Sammon'), names_to = "Method", values_to = "Detection_Rate") %>% 
  ggplot(aes(Features, Detection_Rate, group = Method, color = Method)) + geom_point() + geom_line() + 
  ggtitle("Detection Rates by Mapping Method") + theme_bw() + xlab("Number of 'Influential' Features")+ 
...
## A Note on Sample Size and Dimensionality

Two factors have the potential to greatly impact the computational complexity of generating maps: the dimensionality of the dataset and the sample size considered. The PERMANOVA step in the method can use either a pre-specified number of permutations, or it can be based on a permutation matrix that includes each of the permuted indices as rows.  In small sample size situations, we utilize an exact p-value that is calculated using all of the possible permutations of the indicator variable as the explanatory variable consists only of a single 1 and the rest 0's.  Thus, there are only $n$ distinct permutations to explore.  In larger sample size scenarios, it may not be computationally feasible to consider every possible permutation of the indicator; instead, a raw number of permutations can be calculated and an estimated p-value can be calculated. 


In truly high dimensional data, it is possible that an observation may only differ substantively on a small number (say 1 or 2) of features in a dataset with potentially thousands of dimensions.  The computational cost of PERMANOVA on such a large dataset would be expensive in such a case.  In such a case, it may be useful to consider a method for trimming out the number of features considered.  Using a method such as penalized matrix decomposition or sparse clustering (Witten et al., 2009; Witten and Tibshirani, 2010) to trim noise features might reduce the complexity of the initial feature set used in the map.  Additionally, in the style of t-SNE, a "whitening" process could be used by initializing one of the mapping methods used here with PCA (Van der Maaten, 2010).  Methods for pre-processing the data prior to PERMANOVA-based influence detection is left as an avenue for future research, but might prove the key to computational feasibility on vastly more complex datasets than those considered here.  
...
CC2015_filter <- CC2015 %>% filter(BASIC2015 %in% c(15,16,17)) %>% dplyr::select(PDNFRSTAFF, S.ER.D, NONS.ER.D, FACNUM, HUM_RSD, STEM_RSD, OTHER_RSD, SOCSC_RSD)
#perplexity = 30

#include a plot of the 2015 Carnegie Classification


Rank <- c(1:5)
Name <- rep("Name", 5)

kable(cbind(Rank, Name), col.names = c("", "MDS"))
